...
def gen_data_groups(nb_wk, t_curnum, adj, mobility_mean):
    ...
    for i in t_curnum:
        ...
        ret = np.append(
            ...
            [
                nb_gp,
                dt.spreading_groups(group_array),
                dt.get_mean_group_size(i, group_array),
                mobility_mean[it],
                s,
            ],
        )
...
def run_simul_once(
    ...
    for i in range(0, nb_threads):
        ...
        group_data_mat = gen_data_groups(
            len(t_wkpos[0]), t_curnum, nadj, mobility_mean
        )
        del (t_curnum, nadj)
        del t_wkpos
        print("bp 2")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="number of simuls per batch",
    )
    parser.add_argument(
        "nb_thread_max",
        metavar="max_thread",
        type=int,
        nargs=1,
        help="number of threads used per batch",
    )
    parser.add_argument(
        "coeff_walkers",
        metavar="walkers",
        type=int,
        nargs=1,
        help="coeff of walkers per batch",
    )
    parser.add_argument(
        "nb_iterations",
        metavar="nb_iterations",
        type=int,
        nargs=1,
        help="nb iterations of the simul",
    )
    parser.add_argument(
        "simul_opt",
        metavar="simul parameters",
        type=str,
        nargs=1,
        help="string to specify options of the simul",
    )
